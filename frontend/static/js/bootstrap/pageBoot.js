...
import { EVENTS, UI_LABELS } from "../core/state.js?v=0.1.324";
...
commit-matrix</code></pre>
    ...
    try {
        const { initInfiniteScroll } = await import("../ui/tableRender.js?v=0.1.324");
        if (!window.MATRIX_SYSTEM_EMPTY && !window.MATRIX_INVALID_REPO) {
            const p = new URLSearchParams(window.location.search);
            initInfiniteScroll(p.get("repo") || "commit-matrix", 100);
        }

        const { hub } = await import("../core/eventHub.js?v=0.1.324");
        ...
        console.error("[CommitMatrix] Failed to hook Event Hub in pageBoot.js?v=0.1.324", e);
    }
})();