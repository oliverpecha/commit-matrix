...
def call_model_with_retry_stub(context: dict) -> tuple[str, int]:
    ...
    lines: list[str] = []
    title_suffix = f" @ {commit_sha[:12]}" if commit_sha else ""
    lines.append(f"# Architecture: {repo_name}{title_suffix}")
    lines.append("")
    ...
    lines.append("## Notes")
    lines.append("")
    lines.append("- This blueprint was generated programmatically without an LLM.")
    lines.append("- It reflects repository structure and git history visible at generation time.")
    if commit_sha:
        lines.append(f"- Historical source commit: `{commit_sha}`.")
    lines.append("- Switch to a richer generation mode later for deeper semantic architecture analysis.")
    lines.append("")
