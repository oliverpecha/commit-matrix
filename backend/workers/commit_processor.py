...
def process_commit(i, parts, total_unscanned, processed_count, arch_context, model_name, rubric_path, rate_limits, aimd, arch_tree_signature=None, arch_gen=None, arch_gen_trail=None):
    MAX_RETRIES = 6
    retries = MAX_RETRIES
    ...
    while retries > 0:
        try:
            ...
            headers = ['#', 'Date', 'Type', 'Scope', 'Subject', 'Tier', 'C', 'I', 'R', 'S', 'D', 'Total', 'Additions', 'Deletions', 'Hash', 'ArchSig', 'ArchGen']
            row = [i, date_str, commit_type, commit_scope, subject, tier_label.split()[1], criticality, infrastructure, ripple, scope, documentation, total_score, f'+{additions}', f'-{deletions}', hash_short, arch_tree_signature or "", arch_gen if arch_gen is not None else ""]
