import { CM_COLORS, BP_AXC_BASE, SC_COLORS } from '../core/constants.js?v=2';
import { calcMAvg, getTop25 } from '../core/dataEngine.js?v=2';
import { UI_STATE } from '../core/state.js?v=2';
import { monthDiv, customTooltip, getXConf, MD_TOP } from './plugins.js?v=2';
...
export function renderStackChart(c) {
    if(charts.stack) charts.stack.destroy(); const lin=UI_STATE.stack; const mk=(ax)=>c.map(x=>lin?{x:x.ts,y:x[ax]}:x[ax]);
...
export function renderTrendChart(c) {
    if(charts.trend) charts.trend.destroy(); const lin=UI_STATE.trend; const avg=calcMAvg(c.map(x=>x.tot),UI_STATE.avgTrend,c,lin); const mk=(tr)=>c.map(x=>tr&&x.tier!==tr?(lin?{x:x.ts,y:null}:null):(lin?{x:x.ts,y:x.tot}:x.tot));
...
function buildCombo(id, stKey, avgKey, c, dFunc, clr) {
    if(charts[stKey]) charts[stKey].destroy(); const lin=UI_STATE[stKey]; const avg=calcMAvg(c.map(dFunc),UI_STATE[avgKey],c,lin);
    charts[stKey] = new Chart(id,{type:'bar',data:{labels:lin?undefined:c.map(x=>`#${x.n}`),datasets:[{type:'line',data:avg,borderColor:'rgba(79,152,163,0.8)',borderWidth:1.5,pointRadius:0,tension:0.3},{type:'bar',data:c.map(x=>lin?{x:x.ts,y:dFunc(x)}:dFunc(x)),backgroundColor:clr,borderRadius:2,barThickness:lin?4:undefined}]},options:{...def(c),layout:{padding:{top:MD_TOP}},scales:{x:getXConf(lin,c),y:{...def(c).scales.y,beginAtZero:true}}},plugins:lin?[monthDiv(c)]:[]});
}
...
export function renderConvergenceChart(c) {
    if(charts.conv) charts.conv.destroy(); const lin=UI_STATE.conv; 
    const f=c.map(x=>(x.C+x.R)/(x.D||1)), ch=c.map(x=>x.C/(x.I||1)), b=c.map(x=>x.S*x.R); const fT=getTop25(f), cT=getTop25(ch), bT=getTop25(b);
    const nd=c.map((x,i)=>{ const h=(f[i]>=fT?1:0)+(ch[i]>=cT?1:0)+(b[i]>=bT?1:0); if(h>=2&&x.tot>0){const m=Math.max(f[i],ch[i],b[i]);return lin?{x:x.ts,y:m+1}:m+1;} return lin?{x:x.ts,y:null}:null; });
    charts.conv = new Chart('cm-c-conv',{type:'line',data:{labels:lin?undefined:c.map(x=>`#${x.n}`),datasets:[{label:'Node',data:nd,type:'scatter',pointBackgroundColor:c.map(x=>SC_COLORS['t_'+x.scope]||'#fff'),pointBorderColor:'rgba(255,255,255,0.8)',pointBorderWidth:2,pointRadius:6},{label:'Frag',data:c.map((x,i)=>lin?{x:x.ts,y:f[i]}:f[i]),borderColor:'rgba(255, 75, 75, 0.4)',backgroundColor:'rgba(255, 75, 75, 0.05)',borderWidth:1,fill:true,tension:0.4,pointRadius:0},{label:'Churn',data:c.map((x,i)=>lin?{x:x.ts,y:ch[i]}:ch[i]),borderColor:'rgba(201, 158, 240, 0.4)',backgroundColor:'rgba(201, 158, 240, 0.05)',borderWidth:1,fill:true,tension:0.4,pointRadius:0},{label:'Blast',data:c.map((x,i)=>lin?{x:x.ts,y:b[i]}:b[i]),borderColor:'rgba(255, 184, 77, 0.4)',backgroundColor:'rgba(255, 184, 77, 0.05)',borderWidth:1,fill:true,tension:0.4,pointRadius:0}]},options:{...def(c),layout:{padding:{top:MD_TOP}},scales:{x:getXConf(lin,c),y:{...def(c).scales.y,beginAtZero:true}}},plugins:lin?[monthDiv(c)]:[]});
}
