...
- Git
- An API key for at least one LiteLLM-supported provider (e.g. OpenAI, Anthropic, Gemini)[page:0]
...
This binds a global `commit-matrix` command and prepares the Docker environment via `docker-compose.yml`.[page:0]
...
The tool ingests your history, scores commits, and writes a namespaced ledger into `data/commit-matrix` inside the CommitMatrix project directory.[page:0]
...
If you are running this on a remote host, replace `localhost` with the machine’s IP or hostname.[page:0]
...
Rubrics let CommitMatrix adapt to different types of engineering work without hardcoding a single definition of “good.” Rubrics live under `backend/rubrics/` as Markdown specifications that the parser loads at runtime.[page:0]
...
See `calibration/RUBRIC_AUTHORING_GUIDE.md` for the rubric contract and authoring standards.[page:0] Each rubric defines:
...
- **Backend (Python + LiteLLM)**  
  ...
  - Provider‑agnostic LLM proxy via LiteLLM, so you can switch between OpenAI, Anthropic, Gemini, etc. with configuration only.[page:0]

- **Calibration harness**  
  - 24‑fixture suite under `calibration/` with floor, typical, and adversarial diffs per rubric.  
  - Ensures new rubrics and model settings keep passing a strict JSON contract before touching real repositories.[page:0]

- **Frontend (vanilla ES6)**  
  - `static/js/core/dataEngine.js` ingests CSV/JSON ledgers and infers axes.  
  - `static/js/charts/chartCtrl.js` and `static/js/ui/heatmap.js` render responsive heatmaps, tables, and charts without heavy frameworks.[page:0]

- **CLI wrapper**  
  - `install.sh` and `uninstall.sh` wire up a global `commit-matrix` command that orchestrates containers on demand via `docker-compose.yml`.[page:0]
...
LLMs hallucinate. CommitMatrix assumes that and defends against it with a dynamic calibration harness under `calibration/`.[page:0]
...
- A calibration script (`calibration/calibrate.py`) runs all fixtures and asserts a strict, deterministic contract before allowing a rubric or model configuration to go live.[page:0]
...
CommitMatrix started as an internal tool and is evolving into a standalone, community‑grade telemetry layer for AI‑era engineering teams.[page:0]
...
- Optional CI integrations to surface CommitMatrix signals directly on PRs[page:0]
